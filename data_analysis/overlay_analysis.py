...
def process_bag(bag_path, map_yaml_path):
    ...
    pf_interp_x = interp1d(pf_odom_times, pf_odom_x, kind='linear', 
                       bounds_error=False, fill_value="extrapolate")
    pf_interp_y = interp1d(pf_odom_times, pf_odom_y, kind='linear',
                       bounds_error=False, fill_value="extrapolate")
    ...
    interp_pf_odom_x = pf_interp_x(model_times)
    interp_pf_odom_y = pf_interp_y(model_times)
    ...
    pf_dx = model_x - interp_pf_odom_x
    pf_dy = model_y - interp_pf_odom_y
    pf_distances = np.sqrt(pf_dx**2 + pf_dy**2)
    ...
    pf_avg_distance = np.nanmean(pf_distances)
    pf_max_distance = np.nanmax(pf_distances)
    pf_min_distance = np.nanmin(pf_distances)
    # Plotting
    ...
    plt.plot(pf_odom_x, pf_odom_y, 'g-', label='Particle Filter Estimate', alpha=0.8)
    ...
    print(f"Average distance: {pf_avg_distance:.6f}m")
    print(f"Max distance: {pf_max_distance:.6f}m")
    print(f"Min distance: {pf_min_distance:.6f}m")
