...
raycast_kernel = cp.RawKernel(r'''
extern "C" __global__
void raycast(
    const float* px, const float* py, const float* ptheta,
    const unsigned char* map, const int width, const int height,
    const float resolution, const float origin_x, const float origin_y,
    const float* angles, const int num_angles,
    const float max_range, const float step_size,
    float* out_ranges)
{
    ...
    int pid = tid / num_angles;
    int aid = tid % num_angles;
    float x0 = px[pid], y0 = py[pid];
    float theta = ptheta[pid] + angles[aid];
    float cos_t = cosf(theta), sin_t = sinf(theta);

    // precompute how many steps to march
    int num_steps = int(ceilf(max_range / step_size));
    ...
    out_ranges[pid * num_angles + aid] = range;
}
''', 'raycast')

def gpu_raycast(particles, map_msg, angles, max_range=10.0, step_size=0.05):
    """
    particles: cp.ndarray shape (N,3) [x, y, theta]
    map_msg: nav_msgs/OccupancyGrid
    angles: np.ndarray or cp.ndarray of scan angles (shape M,)
    returns: cp.ndarray shape (N, M) of ranges
    """
    N = particles.shape[0]
    M = angles.shape[0]

    # Upload map to GPU as uint8
    grid = cp.asarray(map_msg.data, dtype=cp.uint8)
    ...
    # Decompose particle arrays
    px = particles[:, 0].astype(cp.float32)
    py = particles[:, 1].astype(cp.float32)
    ptheta = particles[:, 2].astype(cp.float32)

    # Ensure angles on GPU
    ang_gpu = cp.asarray(angles, dtype=cp.float32)

    # Prepare output
    out = cp.empty((N * M,), dtype=cp.float32)

    # Cast all scalar args to the exact C types:
    width_i  = np.int32(map_msg.info.width)
    height_i = np.int32(map_msg.info.height)
    M_i      = np.int32(M)
    res_f    = cp.float32(map_msg.info.resolution)
    ox_f     = cp.float32(map_msg.info.origin.position.x)
    oy_f     = cp.float32(map_msg.info.origin.position.y)
    max_r_f  = cp.float32(max_range)
    step_f   = cp.float32(step_size)
...
class ParticleFilter(Node):
    def __init__(self):
        ...
        self.sigma_hit = 0.2  # Reduced sigma for sharper distribution
        ...
        # State variables
        self.particles = cp.zeros((self.num_particles, 3), dtype=cp.float32)
        self.weights = cp.ones(self.num_particles, dtype=cp.float32) / self.num_particles
        self.map_data = None
        self.map_info = None
        self.last_odom = None
        self.current_odom = None
        # Holder for map message
        self.map_msg = None
    ...
    def initialize_particles(self):
        ...
        xs = cp.random.normal(self.initial_pose_x, self.initial_cov_x * 0.5, self.num_particles)
        ys = cp.random.normal(self.initial_pose_y, self.initial_cov_y * 0.5, self.num_particles)
        thetas = cp.random.normal(self.initial_pose_a, self.initial_cov_a * 0.5, self.num_particles)
        self.particles[:, 0] = xs
        self.particles[:, 1] = ys
        self.particles[:, 2] = cp.mod(thetas + cp.pi, 2*cp.pi) - cp.pi
        # Validate positions (on CPU)
        particles_cpu = cp.asnumpy(self.particles)
        valid = cp.asarray([self.is_valid_position(x, y) for x, y, theta in particles_cpu], dtype=cp.bool_)
        invalid_idx = cp.where(~valid)[0]
    ...
    def motion_update(self):
        ...
        trans = cp.sqrt(dx**2 + dy**2)
        rot1 = cp.arctan2(dy, dx) - theta1 if trans > 0.001 else 0
        rot2 = dtheta - rot1
        ...
        noise_scale = cp.clip(trans * 5 + cp.abs(dtheta) * 5, 0, 1)
        
        # Generate all random values at once
        noise = cp.random.normal(0, 1, (self.num_particles, 3), dtype=cp.float32)
        # noise = np.random.normal(0, 1, (self.num_particles, 3))
        noisy_rot1 = rot1 + (self.alpha1 * cp.abs(rot1) + self.alpha2 * trans) * noise[:, 0] * noise_scale
        noisy_trans = trans + (self.alpha3 * trans + self.alpha4 * (cp.abs(rot1) + cp.abs(rot2))) * noise[:, 1] * noise_scale
        noisy_rot2 = rot2 + (self.alpha1 * cp.abs(rot2) + self.alpha2 * trans) * noise[:, 2] * noise_scale
        
        # Apply motion vectorized
        theta_new = (self.particles[:, 2] + noisy_rot1 + cp.pi) % (2*cp.pi) - cp.pi
        self.particles[:, 0] += noisy_trans * cp.cos(theta_new)
        self.particles[:, 1] += noisy_trans * cp.sin(theta_new)
        self.particles[:, 2] = (theta_new + noisy_rot2 + cp.pi) % (2*cp.pi) - cp.pi
    ...
    def measurement_update(self, scan_msg):
        ...
        # particles_cpu = cp.asnumpy(self.particles)
        # self.get_logger().info("measurement called")
        weights = cp.zeros_like(self.weights)
        angles = np.arange(scan_msg.angle_min, scan_msg.angle_max, scan_msg.angle_increment)
        ranges = np.array(scan_msg.ranges[::10])
        scan_angles = angles[::10]
        sim_ranges = gpu_raycast(self.particles,
                         self.map_msg,
                         scan_angles,
                         max_range=self.laser_max_range,
                         step_size=0.05)
        # now compute likelihood per particle by comparing sim_ranges vs actual ranges vectorized Gaussian:
        # for i in range(0, 10):
        #     self.get_logger().info(f"sim range: {sim_ranges[i]}, actual range {ranges}")
        diff = sim_ranges - cp.asarray(ranges, dtype=cp.float32)[None, :]
        sigma = cp.float32(self.sigma_hit)
        z_hit, z_rand = cp.float32(self.z_hit), cp.float32(self.z_rand)

        # mixture model per beam
        p_hit = z_hit * (1/(sigma*cp.sqrt(2*cp.pi))) * cp.exp(-0.5 * (diff/sigma)**2)
        p_rand = z_rand / self.laser_max_range
        lik = p_hit + p_rand

        # log‐sum for stability
        logL = cp.sum(cp.log(lik + 1e-12), axis=1)
        logL -= cp.max(logL)
        w = cp.exp(logL)
        w /= cp.sum(w)
        ...
        # debug
        w_cpu = w.get()
        self.get_logger().info(f"w: min={w_cpu.min():.3e}, max={w_cpu.max():.3e}")
    ...
    def resample(self):
        """Resample particles based on their weights, fully on GPU"""
        # 1) get indices on CPU (reuse your existing samplers)
        indices = self.low_variance_resampling()   # -> NumPy int array of shape (N,)
        
        # 2) move them to the GPU
        indices_gpu = cp.asarray(indices, dtype=cp.int32)

        # 3) gather old particles & weights
        particles = self.particles[indices_gpu]       # (N,3) on GPU
        weights   = self.weights[indices_gpu]         # (N,) on GPU

        # 4) compute per‐particle noise scale (less noise for high‐weight)
        noise_scale = cp.maximum(0.001, 0.01 * (1.0 - weights))

        # 5) sample zero‐mean unit Gaussian noise and scale it
        noise = cp.random.normal(0, 1, (self.num_particles, 3), dtype=cp.float32)
        noise[:, 0] *= noise_scale                   # x
        noise[:, 1] *= noise_scale                   # y
        noise[:, 2] *= (noise_scale * 0.5)           # θ (smaller rotational noise)

        # 6) apply noise and wrap angles to [–pi, pi)
        particles += noise
        particles[:, 2] = (particles[:, 2] + cp.pi) % (2 * cp.pi) - cp.pi

        # 7) fall back to CPU for validity‐check of positions
        cpu_pts = cp.asnumpy(particles)
        valid   = cp.asarray([self.is_valid_position(x, y) for x, y, _ in cpu_pts], dtype=cp.bool_)
        if not valid.all():
            bad = cp.asnumpy(cp.where(~valid)[0])
            # keep original for any invalid ones
            orig = cp.asnumpy(self.particles[indices_gpu[bad]])
            cpu_pts[bad] = orig

        # 8) store back on GPU and reset weights
        self.particles = cp.asarray(cpu_pts, dtype=cp.float32)
        self.weights   = cp.ones(self.num_particles, dtype=cp.float32) / self.num_particles
    ...
    def get_estimated_pose(self):
        ...
        best_particle_idx = np.argmax(weights_cpu)
        best_x, best_y, best_theta = particles_cpu[best_particle_idx]
        ...
        for i, (px, py, ptheta) in enumerate(particles_cpu):
            w = weights_cpu[i]
            sum_x += px * w
            sum_y += py * w
        ...
        var_x = 0
        var_y = 0
        for i, (px, py, _) in enumerate(particles_cpu):
            w = weights_cpu[i]
            var_x += w * (px - sum_x)**2
            var_y += w * (py - sum_y)**2
        ...
        return sum_x.item(), sum_y.item(), mean_theta.item()
    ...
    def publish_particles(self):
        ...
        for x, y, theta in particles_cpu:
            pose = Pose()
